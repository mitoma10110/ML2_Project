...
def create_agg_clusters(data, linkage='ward', distance_threshold=0, n_clusters=None):
    agg_clust = AgglomerativeClustering(linkage=linkage, distance_threshold=distance_threshold, n_clusters=n_clusters).fit(data)
    return agg_clust
...
def allocate_clusters_hierarchical(df, data_preprocessed, n_clusters, linkage='ward'):
